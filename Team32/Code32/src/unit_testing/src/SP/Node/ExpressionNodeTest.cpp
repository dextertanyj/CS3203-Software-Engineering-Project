...
TEST_CASE("SP::Node::ExpressionNode::equals") {
	shared_ptr<ExpressionNode> node = make_shared<ExpressionNode>(createArithmeticExpression(vector<string>({"A", ";"})));

	SECTION("Same Object Test") { REQUIRE(node->equals(node)); }

	SECTION("Same Node Test") {
		shared_ptr<ExpressionNode> other = make_shared<ExpressionNode>(createArithmeticExpression(vector<string>({"A", ";"})));
	...
	SECTION("Different Node Test") {
		shared_ptr<ExpressionNode> other = make_shared<ExpressionNode>(createArithmeticExpression(vector<string>({"(", "B", ")", ")"})));
