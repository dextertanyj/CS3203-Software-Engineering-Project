...
TEST_CASE("SP::Node::StatementNode::parse") {
	SP::Lexer lex;
	SP::Lexer lex_2;
	...
	SECTION("Assignment Valid Token Test") {
		...
		unique_ptr<StatementNode> node = StatementNode::parse(lex, statement_count);
		unique_ptr<VariableNode> assignee = make_unique<VariableNode>("x");
		unique_ptr<ExpressionNode> expression =
			make_unique<ExpressionNode>(SP::TestUtilities::createArithmeticExpression(vector<string>({"10", "+", "3", ";"})));
		...
		unique_ptr<StatementNode> node_2 = StatementNode::parse(lex_2, statement_count);
		unique_ptr<VariableNode> assignee_2 = make_unique<VariableNode>("read");
		unique_ptr<ExpressionNode> expression_2 =
			make_unique<ExpressionNode>(SP::TestUtilities::createArithmeticExpression(vector<string>({"10", ";"})));
	...
	SECTION("Invalid Keyword Test") {
		...
		REQUIRE_THROWS_AS(StatementNode::parse(lex, statement_count), SP::ParseException);
