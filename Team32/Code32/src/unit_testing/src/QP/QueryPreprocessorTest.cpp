...
TEST_CASE("QP::QueryPreprocessor::parseQuery valid such that Follows(*)") {
	...
	clauseT = dynamic_pointer_cast<QP::Relationship::FollowsT>(qp10.getSuchThatClauseList()[0].relation).get();
	...
	clauseT = dynamic_pointer_cast<QP::Relationship::FollowsT>(qp11.getSuchThatClauseList()[0].relation).get();
	REQUIRE((*clauseT).getLeftStmt().type == StmtRefType::Synonym);
	REQUIRE((*clauseT).getLeftStmt().stmt_ref == "w1");
	REQUIRE((*clauseT).getRightStmt().type == StmtRefType::Synonym);
	REQUIRE((*clauseT).getRightStmt().stmt_ref == "c2");
	...
	clauseT = dynamic_pointer_cast<QP::Relationship::FollowsT>(qp13.getSuchThatClauseList()[0].relation).get();
...
TEST_CASE("QP::QueryPreprocessor::parseQuery valid such that UsesS/P") {
	...
	QP::QueryProperties qp15 = qpp15.parseQuery(univDeclarations + "Select s2 such that Uses(\"procedureName\", \"varName\")");
...
TEST_CASE("QP::QueryPreprocessor::parseQuery valid such that ModifiesS/P") {
	...
	QP::QueryPreprocessor qpp15;
	QP::QueryProperties qp15 = qpp15.parseQuery(univDeclarations + "Select s2 such that Modifies(\"procedureName\", \"varName\")");
...
TEST_CASE("QP::QueryPreprocessor::parseQuery valid pattern") {
	...
	REQUIRE((*clause1).getExpressionType() == ExpressionType::ExpressionUnderscore);
	QP::QueryPreprocessor qpp7;
	QP::QueryProperties qp7 = qpp7.parseQuery(univDeclarations + "Select a1 pattern a1(\"foo\", \"(x+5)*3- y\")");
	...
	REQUIRE((*clause1).getExpressionType() == ExpressionType::Underscore);
	QP::QueryPreprocessor qpp9;
	QP::QueryProperties qp9 = qpp9.parseQuery(univDeclarations + "Select a2 pattern a1(\"foo\", _\"x-2\"_)");
