...
TEST_CASE("QP::QueryEvaluator::splitClauses Should split clauses into groups") {
	...
	vector<pair<SuchThatClauseList, PatternClauseList>> clausesInGroup =
		QP::QueryEvaluator(pkb).splitClauses(properties, synonymsInGroup);
...
TEST_CASE("QP::QueryEvaluator::execute") {
	...
	SECTION("Non-trivial pattern clause and such that clause") {
		SuchThatClauseList suchThatClauses = { { make_unique<QP::Relationship::ModifiesS>(stmtNo1, v)} };
		PatternClauseList patternList = { {make_unique<QP::Relationship::Pattern>(assignSynonym, v, ExpressionType::ExpressionUnderscore, queryExpression)} };
