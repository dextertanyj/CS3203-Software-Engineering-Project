...
TEST_CASE("VariableNode::equals Same Object Test") {
	shared_ptr<ArithmeticProcessor::VariableNode> variable = make_shared<VariableNode>("A");
	REQUIRE(variable->equals(variable));
}

TEST_CASE("VariableNode::equals Same Variable Name Test") {
	shared_ptr<ArithmeticProcessor::VariableNode> variable = make_shared<VariableNode>("A");
	shared_ptr<ArithmeticProcessor::VariableNode> other = make_shared<VariableNode>("A");
	REQUIRE(variable->equals(other));
}

TEST_CASE("VariableNode::equals Different Variable Name Test") {
	shared_ptr<ArithmeticProcessor::VariableNode> variable = make_shared<VariableNode>("A");
	shared_ptr<ArithmeticProcessor::VariableNode> other = make_shared<VariableNode>("B");
	REQUIRE_FALSE(variable->equals(other));
}

TEST_CASE("VariableNode::equals Different Expression Test") {
	shared_ptr<ArithmeticProcessor::VariableNode> variable = make_shared<VariableNode>("A");
	shared_ptr<ArithmeticProcessor::ExpressionNode> other = make_shared<ConstantNode>("123");
