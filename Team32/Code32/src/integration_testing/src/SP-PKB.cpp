...
TEST_CASE("SP::Processor::process Basic Test") {
    ...
    SECTION("Basic If Test") {
        ...
        SECTION("Modify") {
            ...
            std::unordered_set<shared_ptr<StmtInfo>> expected_modify_x = {
                    stmt_map.find(1)->second,
                    stmt_map.find(2)->second,
                    stmt_map.find(4)->second
            };
        ...
        SECTION("Pattern") {
            shared_ptr<Common::ExpressionProcessor::ExpressionNode> root = arithmetic(MathematicalOperator::Plus, variable("y"), constant("1"));
            ...
            shared_ptr<Common::ExpressionProcessor::ExpressionNode> root_2 = arithmetic(MathematicalOperator::Plus, variable("x"), constant("1"));
            unordered_set<VarRef> variables_2 = unordered_set<VarRef>({"x"});
            unordered_set<ConstVal> constants_2 = unordered_set<ConstVal>({1});
            Common::ExpressionProcessor::Expression expression_2 = Common::ExpressionProcessor::Expression(root_2, variables_2, constants_2);
    ...
    SECTION("Complex Test") {
        string source = "procedure main { if ( ((!(x < 5)) && (y > 10)) || (z == 1)) "
                        "then { read x; print y; while ( z < y ) { z = 5 * y + 2; }} else { read x; if (x == a) "
                        "then { print a; read z;} else { x = 3 * a + y + 2; } print x;}}";
        source_processor.process(source);
        unordered_map<StmtRef, shared_ptr<StmtInfo>> stmt_map = pkb.getStmtInfoMap();

        SECTION("General") {
            REQUIRE_EQUALS(pkb.getStatements().size(), 11);
        ...
        SECTION("Follow") {
            REQUIRE_EQUALS(pkb.getFollowerStar(1), std::unordered_set<shared_ptr<StmtInfo>>());
        ...
        SECTION("Pattern") {
            shared_ptr<Common::ExpressionProcessor::ArithmeticNode> first = arithmetic(MathematicalOperator::Times, constant("5"), variable("y"));
            ...
            shared_ptr<Common::ExpressionProcessor::ArithmeticNode> sub_root_1 = arithmetic(MathematicalOperator::Plus, variable("y"), constant("2"));
            Common::ExpressionProcessor::Expression sub_expression_1 = Common::ExpressionProcessor::Expression(sub_root_1, sub_variables_1, sub_constants_1);
            ...
            shared_ptr<Common::ExpressionProcessor::ArithmeticNode> sub_root_2 = arithmetic(MathematicalOperator::Times, constant("5"), variable("y"));
            Common::ExpressionProcessor::Expression sub_expression_2 = Common::ExpressionProcessor::Expression(sub_root_2, sub_variables_2, sub_constants_2);
            REQUIRE(pkb.patternExists("z", sub_expression_2, false));
            REQUIRE_FALSE(pkb.patternExists("z", sub_expression_2, true));
            shared_ptr<Common::ExpressionProcessor::ArithmeticNode> sub_root_2_swap = arithmetic(MathematicalOperator::Times, variable("y"), constant("5"));
            Common::ExpressionProcessor::Expression sub_expression_2_swap = Common::ExpressionProcessor::Expression(sub_root_2_swap, sub_variables_2, sub_constants_2);
            ...
            std::unordered_set<shared_ptr<StmtInfo>> expected_assign_z = {
                    stmt_map.find(5)->second
            };
            ...
            std::unordered_set<shared_ptr<StmtInfo>> expected_assign_x = {
                    stmt_map.find(10)->second
            };
