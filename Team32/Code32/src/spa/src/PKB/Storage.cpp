...
bool PKB::Storage::checkCall(const ProcRef &caller, const ProcRef &callee) {
    return call_store.isRelated(caller, callee);
}
...
vector<pair<shared_ptr<StmtInfo>, VarRef>>
PKB::Storage::getStmtsWithPatternRHS(const Common::ExpressionProcessor::Expression &expression,
                                     bool is_exact_match) {
