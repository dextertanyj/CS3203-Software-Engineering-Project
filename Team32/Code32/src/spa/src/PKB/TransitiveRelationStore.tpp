...
bool PKB::TransitiveRelationStore<TIdent, TInfo, TRelation>::isRelated(TIdent front, TIdent back) const {
    ...
    return (any_of(result.begin(), result.end(),
                   [back](const shared_ptr<TInfo> &ptr) { return ptr->getIdentifier() == back; }));
}
...
PKB::TransitiveRelationStore<TIdent, TInfo, TRelation>::getForward(TIdent ident) const {
...
template<typename TIdent, typename TInfo, class TRelation>
unordered_set<shared_ptr<TInfo>>
PKB::TransitiveRelationStore<TIdent, TInfo, TRelation>::getReverse(TIdent ident) const {
    auto iter = map.find(ident);
    if (iter == map.end()) {
        return {};
    }
...
template<typename TIdent, typename TInfo, class TRelation>
unordered_set<shared_ptr<TInfo>>
PKB::TransitiveRelationStore<TIdent, TInfo, TRelation>::getForwardTransitive(TIdent ident) const {
    auto iter = map.find(ident);
    if (iter == map.end()) {
        return {};
    }
...
template<typename TIdent, typename TInfo, class TRelation>
unordered_set<shared_ptr<TInfo>>
PKB::TransitiveRelationStore<TIdent, TInfo, TRelation>::getReverseTransitive(TIdent ident) const {
    auto iter = map.find(ident);
    if (iter == map.end()) {
        return {};
    }
