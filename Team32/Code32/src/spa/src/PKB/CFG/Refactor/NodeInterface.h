...
public:
	void setUniqueIndex(size_t index) { this->unique_index = index; }
	size_t getUniqueIndex() { return this->unique_index; }
	string getNodeType() { return this->node_type; }

	void insertNext(shared_ptr<PKB::NodeInterface> next) { this->next_nodes.push_back(next); }

	void insertPrevious(shared_ptr<PKB::NodeInterface> next) { this->previous_nodes.push_back(next); }
