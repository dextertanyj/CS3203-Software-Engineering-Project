...
struct LessComparator {
    bool operator()(const shared_ptr<PKB::StatementNode> &lhs, const shared_ptr<PKB::StatementNode> &rhs) const {
        return *lhs < *rhs;
    }
};
...
bool PKB::NextManager::checkNextStarOptimized(const shared_ptr<StatementNode> &first_node,
                                              const shared_ptr<StatementNode> &second_node) {
...
priority_queue<shared_ptr<PKB::StatementNode>, vector<shared_ptr<PKB::StatementNode>>, Comparator>
PKB::NextManager::constructQueue(
...
template<class Comparator>
void PKB::NextManager::constructQueueIteration(const shared_ptr<StatementNode> &node,
                                               QueueConstructionInformation<Comparator> &info) {
...
template<class Comparator>
void PKB::NextManager::constructQueueLoopNode(const shared_ptr<StatementNode> &node,
                                              QueueConstructionInformation<Comparator> &info) {
    ...
    if (info.origin->getNodeRef() > node->getNodeRef() &&
        (outer_nodes.empty() || all_of(outer_nodes.begin(), outer_nodes.end(),
                                       [&](const auto &outer) {
                                           return outer->getNodeRef() > info.origin->getNodeRef();
                                       }))) {
...
PKB::NextManager::LoopNodePair PKB::NextManager::processLoopEntryExit(
        const shared_ptr<PKB::StatementNode> &loop_node,
...
PKB::NextManager::LoopNodePair PKB::NextManager::processLoopEntry(const shared_ptr<PKB::StatementNode> &node) {
    return processLoopEntryExit<std::greater<StmtRef>>(node, &NodeInterface::getPreviousNodes,
                                                       &ControlFlowGraph::collectPreviousOfDummy);
}

PKB::NextManager::LoopNodePair PKB::NextManager::processLoopExit(const shared_ptr<PKB::StatementNode> &node) {
    auto result = processLoopEntryExit<std::less<StmtRef>>(node, &NodeInterface::getNextNodes,
                                                           &ControlFlowGraph::collectNextOfDummy);
